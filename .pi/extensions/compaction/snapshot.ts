/**
 * Compaction Snapshot (domínio compaction)
 *
 * Persiste o summary cumulativo produzido pelo custom.ts em disco, com
 * upsert por sessão. Cache LOCAL EFÊMERO — vida da sessão.
 *
 * Não sincroniza pro recall-core. Decisões duradouras vão pelo canal
 * separado recall_save (manual/curado). Embedar conversa inteira no banco
 * gera ruído (lição do experimento Jina) — este cache serve queries
 * quentes via subagent curator durante a sessão ativa, e basta.
 *
 * Layout em disco: <pkg>/.pi/compact-session/<session-id>/
 *   ├── snapshot.md   ← markdown do summary cumulativo (upsert)
 *   └── state.json    ← metadados (compactionCount, tokensBefore, projectId)
 *
 * NOTA: compaction/ tem a MESMA profundidade que o antigo
 * compaction-snapshot/ (.pi/extensions/<dir>), então SNAPSHOT_ROOT
 * resolve para o mesmo caminho — sem ajuste de níveis.
 *
 * Registrado pelo index.ts do domínio via registerCompactionSnapshot(pi).
 */
...
export function registerCompactionSnapshot(pi: ExtensionAPI) {
