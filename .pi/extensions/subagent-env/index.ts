...
interface SingleResult {
	agent: string;
	agentSource: "extension" | "user" | "project" | "unknown";
	task: string;
	exitCode: number;
	messages: Message[];
	stderr: string;
...
function setWorkflowHud(
	ctx: ExtensionContext,
	title: string,
	lines: string[],
	tone: "accent" | "success" | "warning" | "error" | "muted" = "accent",
): void {
	if (!ctx.hasUI) return;
	const theme = ctx.ui.theme;
	ctx.ui.setStatus("subagent-hud", theme.fg(tone, title));
	ctx.ui.setWidget("subagent-hud", lines);
}

function clearWorkflowHud(ctx: ExtensionContext, finalStatus?: string): void {
	if (!ctx.hasUI) return;
	if (finalStatus) ctx.ui.setStatus("subagent-hud", finalStatus);
...
function getPiInvocation(args: string[]): { command: string; args: string[] } {
	const overrideBin = process.env.PI_SUBAGENT_BIN;
	if (overrideBin) {
		const [command, ...extraArgs] = overrideBin.split(/\s+/).filter(Boolean);
		if (!command) return { command: process.execPath, args };
		if (fs.existsSync(command)) {
			return { command: process.execPath, args: [command, ...extraArgs, ...args] };
		}
...
async function runSingleAgent(
	...
	if (!agent) {
		...
		appendSystemLog("subagent:runner", "run_unknown_agent", {
			cwd: cwd ?? defaultCwd,
			agent: agentName,
			step,
			available,
	...
	try {
		appendSystemLog("subagent:runner", "run_start", {
			cwd: cwd ?? defaultCwd,
			agent: agent.name,
			agentSource: agent.source,
			model: agent.model,
		...
		appendSystemLog("subagent:runner", "run_end", {
			...
			stderrPreview: currentResult.stderr.slice(0, 300),
		...
		if (
			...
			appendSystemLog("subagent:runner", "run_end_error", {
				...
				stopReason: currentResult.stopReason,
				errorMessage: currentResult.errorMessage,
				stderrPreview: currentResult.stderr.slice(0, 500),
			});
		}
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const chain = params.chain as Array<{ agent: string; task: string; cwd?: string }> | undefined;
			const tasks = params.tasks as Array<{ agent: string; task: string; cwd?: string }> | undefined;
			...
			appendSystemLog("subagent:tool", "tool_execute_start", {
				cwd: ctx.cwd,
				agentScope,
				hasSingle,
				hasTasks,
			...
			if (chain && chain.length > 0) {
				...
				let previousOutput = "";
				showHud(
					`Chain · 0/${chain.length}`,
					[
						theme.fg("muted", `Mode: chain`),
						theme.fg("dim", `Steps: ${chain.length}`),
				...
				for (let i = 0; i < chain.length; i++) {
						...
						? (partial) => {
								...
								if (currentResult) {
									...
									showHud(
										...
										[
											theme.fg("muted", `Agent: ${step.agent}`),
											theme.fg("dim", `Task: ${summarizeText(taskWithContext)}`),
											theme.fg(
												"dim",
												summarizeText(getFinalOutput(currentResult.messages) || currentResult.stderr || "(running...)", 90),
											),
										],
										"accent",
									);
					...
					if (isError) {
						...
						finishHud(`Chain failed at step ${i + 1}`, "error");
						appendSystemLog("subagent:usage", "subagent_used", {
							cwd: ctx.cwd,
							mode: "chain",
							succeeded: false,
							failedAtStep: i + 1,
							agentNames: results.map((r) => r.agent),
						});
						return {
				...
				finishHud(`Chain complete · ${chain.length}/${chain.length}`, "success");
				appendSystemLog("subagent:usage", "subagent_used", {
					cwd: ctx.cwd,
					mode: "chain",
					succeeded: true,
			...
			if (tasks && tasks.length > 0) {
				...
				showHud(
					`Parallel · 0/${tasks.length}`,
					[
						theme.fg("muted", "Mode: parallel"),
						theme.fg("dim", `Tasks: ${tasks.length}`),
						theme.fg("warning", "Starting..."),
					],
					"accent",
				);
				...
				const emitParallelUpdate = () => {
					...
					const active = allResults.find((r) => r.exitCode === -1);
					showHud(
						`Parallel · ${done}/${allResults.length}`,
						[
							theme.fg("muted", `Mode: parallel`),
							theme.fg("dim", `Done: ${done}/${allResults.length}`),
							theme.fg("dim", `Running: ${running}`),
							active
								? theme.fg("warning", `Active: ${active.agent}`)
								: theme.fg("success", `All tasks finished`),
						],
						running > 0 ? "warning" : done === allResults.length ? "success" : "accent",
					);
					if (onUpdate) {
						onUpdate({
							content: [
				...
				finishHud(
					successCount === results.length ? `Parallel complete · ${successCount}/${results.length}` : `Parallel finished · ${successCount}/${results.length}`,
					successCount === results.length ? "success" : "warning",
				);
				appendSystemLog("subagent:usage", "subagent_used", {
					cwd: ctx.cwd,
					mode: "parallel",
					succeeded: successCount === results.length,
					successCount,
			...
			if (params.agent && params.task) {
				...
				showHud(
					`Single · ${params.agent}`,
					[
						theme.fg("muted", "Mode: single"),
						theme.fg("dim", `Agent: ${params.agent}`),
						theme.fg("dim", `Task: ${summarizeText(params.task)}`),
						...
						if (currentResult) {
							showHud(
								...
								[
									theme.fg("muted", "Mode: single"),
									theme.fg("dim", `Agent: ${params.agent}`),
									theme.fg("dim", summarizeText(getFinalOutput(currentResult.messages) || currentResult.stderr || "(running...)")),
									theme.fg("warning", "Running..."),
								],
								"accent",
							);
				...
				if (isError) {
					const errorMsg =
						result.errorMessage || result.stderr || getFinalOutput(result.messages) || "(no output)";
					finishHud(`Single failed · ${params.agent}`, "error");
					appendSystemLog("subagent:usage", "subagent_used", {
						cwd: ctx.cwd,
						mode: "single",
						succeeded: false,
						agentNames: [result.agent],
				...
				finishHud(`Single complete · ${params.agent}`, "success");
				appendSystemLog("subagent:usage", "subagent_used", {
					cwd: ctx.cwd,
					mode: "single",
					succeeded: true,
			...
			appendSystemLog("subagent:tool", "tool_execute_end", {
				cwd: ctx.cwd,
				agentScope,
